...
network.fc = nn.Linear(network.fc.in_features, n_classes)
network.AuxLogits.fc = nn.Linear(network.AuxLogits.fc.in_features, n_classes)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(train_loader)):        
        ...
        aux_output = output.aux_logits  # Auxiliary output
        loss = criterion(main_output, target) + 0.4 * criterion(aux_output, target)      
        loss.backward()     
        optimizer.step()
        optimizer.zero_grad()
        ...
        pred = main_output.argmax(dim=1).cpu().squeeze()
