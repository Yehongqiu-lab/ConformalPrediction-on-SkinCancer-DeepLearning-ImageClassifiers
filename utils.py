...
def img_portray(opt='Acc',write=True,
    ...
    plt.title(img_title)
    plt.xlabel("Epochs")
    if opt == 'Acc' and dates is not None:
    ...
    elif opt == 'Loss' and dates is not None:
        epoch_nums= np.arange(len(dates))
...
def confusion_matrix(evl_result,n_cla,cla_dict,data=[],kn=None,img_title=None,suf=None):
